...
const PROMPT_RECORDS = [
  {
    ...
    created_at: "2026-06-25 14:30",
    updated_at: "2026-06-26 10:15",
    source_type: "web"
  },
  {
    id: "prompt-002",
    prompt_category: "行銷文案",
    prompt_text: "請幫我針對以下產品功能寫 3 組不同的 Facebook 廣告文案。每組需包含標題、痛點切入的內文，以及行動呼籲 (CTA)。語氣要活潑、吸睛，並加入適當的 Emoji：\n\n[請描述產品功能]",
    created_at: "2026-06-24 09:20",
    updated_at: "2026-06-24 09:20",
    source_type: "line"
  },
  {
    id: "prompt-003",
    prompt_category: "寫作生成",
    prompt_text: "請將以下草稿改寫為一篇結構清晰的部落格文章。要求：\n1. 採用前導、主體、總結的三段式結構\n2. 使用 Markdown 格式，適度加上小標題\n3. 語氣要專業但平易近人，適合一般大眾閱讀：\n\n[請貼入草稿]",
    created_at: "2026-06-23 16:45",
    updated_at: "2026-06-23 16:45",
    source_type: "line"
  },
  {
    id: "prompt-004",
    ...
    created_at: "2026-06-22 11:10",
    updated_at: "2026-06-23 09:00",
    source_type: "web"
  },
  {
    id: "prompt-005",
    ...
    updated_at: "2026-06-22 08:00",
    source_type: "line"
  }
];

function sortPromptsDesc(prompts) {
  return [...prompts].sort((a, b) => b.created_at.localeCompare(a.created_at));
}

function getOrderedPrompts() {
  return sortPromptsDesc(PROMPT_RECORDS);
}

function getPromptById(id) {
  return PROMPT_RECORDS.find((item) => item.id === id) || null;
}

function getPromptIndexById(id, prompts) {
  return prompts.findIndex((item) => item.id === id);
}

const PROMPT_CATEGORIES = ["寫作生成", "行銷文案", "工作效率", "開發技術", "其他"];
